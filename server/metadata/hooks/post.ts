  ...
  try {
    ...
    if (post.access === "PRIVATE" && post.assignedTo) {
      ...
      if (
        ...
        !notifiedUserIds.has(assignedLeader._id.toString())
      ) {
...
mercury.hook.after("UPDATE_POST_RECORD", async function (this: any) {
  try {
    const ctxUser = this.options.ctx.user;
    const prevRecord = this.prevRecord;
    const data = this.data;
    const postId: string = prevRecord._id.toString();
    if (data.comment) {
      const updatePost = await mercury.db.Post.update(
        postId,
        {
          $set: {
    ...
    if (this.record.status === "Accepted") {
      ...
      const acceptedUser: any = await mercury.db.User.get(
        { _id: acceptedUserId },
        { id: ctxUser.id, profile: ctxUser.profile }
      );
      if (user?.token) {
