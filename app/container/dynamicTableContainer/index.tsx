...
function DynamicTableContainer() {
  ...
  const [columns, setColumns] = useState<any>();
    ...
    useEffect(() => {
        getAllModelFields(`query ListModelFields($where: whereModelFieldInput, $limit: Int!) {
          ...
          (async () => {
            ...
            for (const field of data?.listModelFields?.docs || []) {
              if (field.type === "relationship" || field.type === "virtual") {
                refKeyMap[field.name] = await getModelFieldRefModelKey(
                  field.ref
                );
              }
            }
    
            const columns = data?.listModelFields?.docs?.map(
              (field: any) => {
                switch (field?.type) {
                  case "string":
                  case "number":
  ...
  return (
    <div>
      {columns?.length>0?<DynamicTable           data={listModelDataResponse.data?.[`list${model}s`]?.docs || []}
    columns={columns}/>:<Text>Loading...</Text>}</div>
  )
}
