...
const UpdateDynamicRecord = () => {
    ...
    const form = useForm<FormSchema>({
        ...
        defaultValues: data?.listModelFields?.docs.reduce((acc: any, field: any) => {
        ...
        if (DynamicGetQuaryResponse?.data) {
            ...
            data.listModelFields?.docs.forEach((item: any) => {
                if (item.type === "relationship") {
                    if (item.many == true) {
                        form.setValue(item.name, DynamicGetQuaryResponse?.data?.[`get${model}`]?.[item.name]?.map((item: any) => item?.id))
                    }
                    else
                        form.setValue(item.name, DynamicGetQuaryResponse?.data?.[`get${model}`]?.[item.name]?.id)
                    return;
                }
                form.setValue(item.name, DynamicGetQuaryResponse?.data?.[`get${model}`]?.[item.name])
            })
