...
const GenerateMultiRelationshipItems = ({
  fieldData,
  form,
}: {
  fieldData: any;
  ...
  useEffect(() => {
    if (data) {
      const formattedOptions = data?.[`list${fieldData.ref}s`]?.docs.map(
        (item: any) => ({
          id: item.id,
      ...
      setSelectedValues(
        formattedOptions?.filter((item: any) => prevData.includes(item?.id))
      );
    }
  }, [data, error]);
